...
def test_execute_sends_tokens_and_reward(
...
def test_execute_pays_reward_without_liquidity(donation_streamer, mock_pool, tokens, donor, caller):
    token0, token1 = tokens
    amounts = [1, 1]
    _mint_and_approve(token0, donor, donation_streamer.address, amounts[0])
    _mint_and_approve(token1, donor, donation_streamer.address, amounts[1])
    ...
    assert boa.env.get_balance(caller) == caller_balance + reward_per_period
    assert token0.balanceOf(mock_pool.address) == pool_balance0
    assert token1.balanceOf(mock_pool.address) == pool_balance1
